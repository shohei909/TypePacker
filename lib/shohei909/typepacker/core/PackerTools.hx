...
class PackerTools
{
    ...
    macro
    #end
    public static function toTypeInfomation(e:Expr):Expr {
        ...
        var infoType:ComplexType = macro : shohei909.typepacker.core.PortableTypeInfomation<$complexType>;

        return {
            expr : ECheckType(macro PackerTools.resolveType($v{name}), infoType),
            pos : pos,
        }
    }
    ...
    private static function getResourceIdentifer(name:String) {
        var str = Type.getClassName(PackerTools) + ":Type:" + name;
